...
class BillingServiceImpl @Inject constructor(
    ...
    override suspend fun launchBillingFlow(activity: Activity, product: SubscriptionProduct): BillingResult {
        return suspendCancellableCoroutine { continuation ->
    ...
    override fun getSubscriptionStatus(): Flow<SubscriptionStatus> {
        return _subscriptionStatus.asStateFlow()
    }
    ...
    override fun onPurchasesUpdated(billingResult: com.android.billingclient.api.BillingResult, purchases: MutableList<Purchase>?) {
        when (billingResult.responseCode) {
            BillingClient.BillingResponseCode.OK -> {
